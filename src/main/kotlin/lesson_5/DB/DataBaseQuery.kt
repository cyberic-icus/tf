...
import lesson_5.DAO.DAO
import java.sql.ResultSet
import kotlin.text.StringBuilder

class DataBaseQuery(private val mainTable: Table) {
    ...
    fun insert(value: DAO) = insertList(listOf(value))

    fun insertList(values: List<DAO>) {
        DataBaseSevice(databaseName).use { dbService ->
            kotlin.runCatching {
    ...
    fun selectAll(): List<String> {
        return DataBaseSevice(databaseName).use { dbService ->
            runCatching {
    ...
    fun selectByID(id: Int) = selectByIdFrom(id, this.mainTable)
